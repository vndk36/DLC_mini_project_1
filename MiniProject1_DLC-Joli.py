...
def valSet(train_input, train_target,sizeVal):
    ...
    #Output is a new "train_input" (of the size (train_input.size(0)-sizeVal)xtrain_input.size(1)xtrain_input.size(2)) and a "validation_set" whose first dimension has a size of sizeVal
...
def load_data():
    #Goal: download (if necessary) the dataset and separate randomly a validation set from the training set. Create "fake data" to be added to the train set. Demean and set the standard deviation of the different dataset created to 1
...
class NetCONV2D3(nn.Module): #similar to oldanddumb but in 2D
    def __init__(self, nb_hidden):
...
for i in range(0,nb_training):
    ...
    train_error.append(compute_nb_errors(model, train_input, train_target, 50) / train_input.size(0) * 100)
...
for i in range(0,nb_training):
    ...
    #Best result in the other model, here it has a bit worst results
    #train_error AVG 8.81% validation_set_error AVG 20% test_error AVG 22%
    #train_error BEST 7.33% validation_set_error BEST 12.50% test_error BEST 18.00%
...
for i in range(0,nb_training):
    ...
    plt.ylabel('Loss')
    plt.show()
    plt.savefig('fig{:.02f}.png'.format(i))
    print('train_error AVG {:.02f}% validation_set_error AVG {:.02f}% test_error AVG {:.02f}%'.format(
        numpy.asarray(train_error).mean(), 
        numpy.asarray(validation_error).mean(), 
        numpy.asarray(test_error).mean()))
    ...
    #test pour le rapport plus gros réseau -> moins bonne perf
    #train_error AVG 10.48% validation_set_error AVG 20.54% test_error AVG 24.29%
    #train_error BEST 7.67% validation_set_error BEST 12.50% test_error BEST 21.00%
